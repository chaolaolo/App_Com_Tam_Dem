...
exports.editCategory = async(req,res)=>{
    ...
    const { name } = req.body; // Category data from request body
...
exports.getCart = async (req, res) => {
    ...
    try {
        const cart = await Cart.findOne({ user: userId }).populate('items.product','name price image_url quantity');
...
exports.getOrder = async (req, res) => {
    ...
    try {
        const orders = await Order.find({ user: userId }).populate('items.product','name price image_url').sort({ createdAt: -1 });
